...
CNavMesh::CNavMesh(uint16_t zoneID)
: m_zoneID(zoneID)
, m_navMesh(nullptr)
{
...
void CNavMesh::outputError(uint32_t status)
{
    /*
...
std::vector<position_t> CNavMesh::findPath(const position_t& start, const position_t& end)
{
    ...
    if (pathCount > 0)
    {
        int32_t straightPathCount = MAX_NAV_POLYS * 3;
...
std::pair<int16_t, position_t> CNavMesh::findRandomPosition(const position_t& start, float maxRadius)
{
    //TracyZoneScoped;
    dtStatus status;
...
bool CNavMesh::onSameFloor(const position_t& start, float* spos, const position_t& end, float* epos, dtQueryFilter& filter)
{
    ...
    else if (verticalDistance > verticalLimit)
    {
        ...
        uint8_t           verticalLimitTrunc = static_cast<uint8_t>(verticalLimit);
        float           height;
        ...
        for (int i = 0; i < polyCount; i++)
        {
            ...
            if (!dtStatusFailed(status))
            {
                ...
                uint8_t rounded = static_cast<uint8_t>(height) + abs((static_cast<uint8_t>(height) % verticalLimitTrunc) - verticalLimitTrunc);
        ...
        if (heights.size() > 1)
        {
            auto startHeight = static_cast<uint8_t>(spos[1]) + abs((static_cast<uint8_t>(spos[1]) % verticalLimitTrunc) - verticalLimitTrunc);
            auto endHeight   = static_cast<uint8_t>(epos[1]) + abs((static_cast<uint8_t>(epos[1]) % verticalLimitTrunc) - verticalLimitTrunc);
