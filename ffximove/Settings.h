...
public:
    ...
    {
        char buffer[1024];
        sprintf_s(buffer, 1024, "%sconfig\\%s\\default%s", m_AshitaCore->GetAshitaInstallPathA(), PluginName, Extension);
    ...
    {
        char buffer[1024];
        sprintf_s(buffer, 1024, "%sconfig\\%s\\%s%s", m_AshitaCore->GetAshitaInstallPathA(), PluginName, PlayerName, Extension);
    ...
    std::string GetInputSettingsPath(const char* Input)
    {
        ...
        sprintf_s(buffer, 1024, "%sconfig\\%s\\%s", m_AshitaCore->GetAshitaInstallPathA(), PluginName, Input);
        if (std::filesystem::exists(buffer))
            return std::string(buffer);

        sprintf_s(buffer, 1024, "%sconfig\\%s\\%s%s", m_AshitaCore->GetAshitaInstallPathA(), PluginName, Input, Extension);
        if (std::filesystem::exists(buffer))
            return std::string(buffer);
    ...
    std::string GetInputSettingsPath(const char* PlayerName, const char* Input)
    {
        ...
        sprintf_s(buffer, 1024, "%sconfig\\%s\\%s\\%s", m_AshitaCore->GetAshitaInstallPathA(), PluginName, PlayerName, Input);
        if (std::filesystem::exists(buffer))
            return std::string(buffer);

        sprintf_s(buffer, 1024, "%sconfig\\%s\\%s\\%s%s", m_AshitaCore->GetAshitaInstallPathA(), PluginName, PlayerName, Input, Extension);
        if (std::filesystem::exists(buffer))
    ...
    std::string GetInputSettingsPath(uint16_t CurrentZone, const char* Input)
    {
        ...
        sprintf_s(buffer, 1024, "%sconfig\\%s\\%u\\%s", m_AshitaCore->GetAshitaInstallPathA(), PluginName, CurrentZone, Input);
        if (std::filesystem::exists(buffer))
            return std::string(buffer);

        sprintf_s(buffer, 1024, "%sconfig\\%s\\%u\\%s%s", m_AshitaCore->GetAshitaInstallPathA(), PluginName, CurrentZone, Input, Extension);
        if (std::filesystem::exists(buffer))
            return std::string(buffer);
    ...
    std::string GetInputWritePath(const char* Input)
    {
        ...
        if (strstr(Input, ".") == Input + (strlen(Input) - 4))
            sprintf_s(buffer, 1024, "%s\\config\\%s\\%s", m_AshitaCore->GetAshitaInstallPathA(), PluginName, Input);
        else
            sprintf_s(buffer, 1024, "%s\\config\\%s\\%s.xml", m_AshitaCore->GetAshitaInstallPathA(), PluginName, Input);
